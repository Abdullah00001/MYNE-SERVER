...
async def identify_upload(
    ...
    async def enrich_match(match):
        ...
        color = match.get("detectedColors", [""])[0]
        leather = match.get("detectedLeathers", "")
        full_query = match.get("imageSearchQuery", "")

        queries = [
            full_query,
            f"{brand} {model} {color} {leather}",
            f"{brand} {model} {color}",
        ]
        ...
        for query in queries:
            ...
            if image_data.get("imageUrl"):
                return {**match, "imageUrl": image_data["imageUrl"], "thumbnailUrl": image_data.get("thumbnailUrl", "")}

        return {**match, "imageUrl": "", "thumbnailUrl": ""}
